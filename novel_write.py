...
def cmd_usage(args):
    ...
    print(f"   https://aistudio.google.com/billing?billing=0110F7-F80EAE-55DB29&project=gen-lang-client-0043335542")
...
def parse_outline(outline_path: Path) -> dict[int, str]:
    """
    解析章節綱要 md，回傳 {章節號: prompt文字} 的字典。
    支援兩種格式：
      #### 第N章 【標題】           ← 只有標題，prompt = 標題
      #### 第N章 【標題】\n內文...  ← 有詳細描述，prompt = 標題 + 內文
    """
    text = outline_path.read_text(encoding="utf-8")
    result = {}

    # 用 regex 找所有章節條目
    # 格式：#### 第N章 【...】 後面可能跟著內文直到下一個 ####
    pattern = re.compile(
        r'####\s+第\s*(\d+)\s*章\s*【([^】]+)】(.*?)(?=####|\Z)',
        re.DOTALL
    )
    ...
    for m in pattern.finditer(text):
        ...
        result[ch_num] = prompt

    return result

def get_nav_links(chapter_num: int) -> str:
    """產生跨卷的上一章/下一章 Markdown 導覽連結"""
    links = []
...
def cmd_batch(args):
    """
    批次生成多個章節。
    從綱要 md 自動讀取每章的 prompt，依序生成並存檔。
    已存在的章節預設跳過（用 --overwrite 強制覆蓋）。
    """
    check_api_key()
    ...
    for n in targets:
        ...
        ch_path = ch_path_info["chapter_dir"] / f"{n:03d}.md"
    ...
    for idx, ch_num in enumerate(todo, 1):
        ...
        ch_path = ch_path_info["chapter_dir"] / f"{ch_num:03d}.md"
        ...
        for i in range(max(1, ch_num - args.prev), ch_num):
            ...
            p = p_info["chapter_dir"] / f"{i:03d}.md"
            if p.exists():
...
def cmd_write(args):
        ...
        try:
            ...
            if chapter_num in outline:
                if not chapter_prompt:
                    ...
                    print(f"   📖 已從綱要讀取第 {chapter_num} 章的提示語")
                else:
                    print(f"   📖 檢測到手動 prompt，將與綱要合併")
                    chapter_prompt = f"{outline[chapter_num]}\n\n手動補充提示：\n{chapter_prompt}"
    ...
    for i in range(max(1, chapter_num - prev_count), chapter_num):
        p_info = get_path_info(i)
        ch_path = p_info["chapter_dir"] / f"{i:03d}.md"
        if ch_path.exists():
...
"""
    ...
    # 輸出
    out_path = Path(args.output) if args.output else path_info["chapter_dir"] / f"{chapter_num:03d}.md"
    if args.dry_run:
        print(text)
    else:
...
def cmd_expand(args):
    ...
    chapter_num = int(args.chapter)
    path_info = get_path_info(chapter_num)
    ch_path = path_info["chapter_dir"] / f"{chapter_num:03d}.md"
...
"""
    ...
    out_path = PROJECT_ROOT / f"synopsis_{start:03d}-{end:03d}.md"
    out_path.write_text(text, encoding="utf-8")
